...
pub struct MarketplaceTransactionsSoA {
    ...
    pub dispatch_margins_hours: Vec<u8>,
    pub is_layaway: Vec<bool>,
    pub sha256_signatures: Vec<String>,
}
...
fn ingest_data(file_path: &str) -> Result<(), Box<dyn Error>> {
    println!("Iniciando ingesta determinista hacia el núcleo de DatioLabs...");
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() < 2 {
        println!("DatioLabs Driver CSV (Marketplace Simulator)");
        println!("Uso: {} <comando> [opciones]", args[0]);
        println!("Comandos:");
        println!("  --generate <cantidad> <archivo.csv>  Genera histórico de datos externo.");
        println!("  --ingest <archivo.csv>               Consume el archivo hacia DatioLabs Core.");
        return Ok(());
    }
