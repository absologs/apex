...
Este documento compila de forma exhaustiva los modelos matemáticos, ecuaciones y algoritmos que fundamentan el motor de inferencia económica y optimización de precios (Dynamic Pricing) del sistema Apex. El sistema modela el inventario de la empresa como una variedad topológica discreta, abandonando la contabilidad tradicional escalar a favor de un cálculo tensorial iterativo bajo restricciones estocásticas de hiperinflación y entropía (fricción antrópica).
...
- $\epsilon_{entropia}$: Fricción del lote (robos, mermas físicas identificadas empíricamente, errores de conteo). Cualquier perturbación antrópica en un lote altera asimétricamente la presión de reposición del inventario restante.

---

## 2. Problema de Optimización: El Precio de Supervivencia ($P_{floor}$)

El límite inferior de precio ("Precio de Supervivencia") no es un margen porcentual fijo ni un _mark-up_ ingenuo. Se define formalmente como la solución a un problema de optimización con restricciones (Boundary Condition) mediante un proceso de salto-difusión.

El límite inferior absoluto e inquebrantable $P_{floor}^{(i)}$ para el producto $i$ en el instante $t$ se formula como:

$$
P_{floor}^{(i)}(t) = \inf \left\{ p \in \mathbb{R}^+ \mid \mathbb{E}^{\mathbb{Q}} \left[ \frac{p \cdot (1 - \mu_i)}{C_{repo}^{(i)}(t + 	au)} ight] \ge 1 + \mathcal{R} ight\}
$$
...
- $C_{repo}^{(i)}(t+	au)$: El costo de reposición estocástico (no el costo histórico) que regirá en el momento incierto de la recompra del lote in $t+	au$.
...
$$
H_i(t) = \frac{\partial V_{sales}^{(i)}}{\partial t} \cdot \left( \frac{P_{actual}^{(i)} - C_{repo}^{(i)}}{C_{repo}^{(i)} } ight) \cdot \frac{1}{\pi_{inflacion}}
$$
...
- $\left( \frac{P_{actual}^{(i)} - C_{repo}^{(i)}}{C_{repo}^{(i)}} ight)$: El margen de protección real actual frente al costo de reposición $C_{repo}$ (no frente al costo histórico contable).
...
Si el vector $H_i(t) \le 0$, indica matemáticamente que la derivada del flujo de caja real se ha vuelto negativa; el activo cruza la métrica de frontera convirtiéndose en un sumidero de liquidez parasitario. En este estado, el sistema emite una directiva ineludible: liquidar de inmediato la totalidad del lote remanente acercándolo a $P_{floor}$ para reciclar el capital en SKUs viables.